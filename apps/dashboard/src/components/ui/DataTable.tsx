      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-stone-200">
          <thead className="bg-stone-50">
            <tr>
              {columns.map((col) => (
                <th key={col.key} className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-stone-500 md:px-6">
                  {col.header}
                </th>
              ))}
            </tr>
            ...
            {isLoading ? (
              ...
              data.map((row) => (
                <tr key={keyExtractor(row)} className="hover:bg-stone-50">
                  {columns.map((col) => (
                    <td key={col.key} className="whitespace-nowrap px-4 py-4 text-sm text-stone-900 md:px-6">
